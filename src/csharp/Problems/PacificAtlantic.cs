...
public sealed class PacificAtlantic : ProblemBase
{
    ...
    private IList<IList<int>> Solution(Matrix heights)
    {
        ...
        var map = new Oceans?[heights[0].Length, heights.Length];

        for (var y = 0; y < heights.Length; y++)
        {
            for (var x = 0; x < heights[0].Length; x++)
            {
                if (Flow(heights, x, y, map) == Oceans.Both)
                {
                    result.Add(new[] {y, x});
                }
            }
    ...
    private Oceans? Flow(int[][] heights, int x, int y, Oceans?[,] map)
    {
        if (map[x, y] != null)
        {
            return map[x, y];
        ...
        map[x, y] = Oceans.None;
        if ((y == heights.Length - 1 && x == 0) || (y == 0 && x == heights[0].Length - 1))
        {
            map[x, y] = Oceans.Both;
        }
        else if (x == 0 || y == 0)
        {
            map[x, y] = Oceans.Pacific;
        }
        else if (x == heights[0].Length - 1 || y == heights.Length - 1)
        {
            map[x, y] = Oceans.Atlantic;
        }

        var current = heights[y][x];
        if (x - 1 >= 0 && current >= heights[y][x - 1])
        {
            map[x, y] |= Flow(heights, x - 1, y, map);
        }

        if (x + 1 < heights[0].Length && current >= heights[y][x + 1])
        {
            map[x, y] |= Flow(heights, x + 1, y, map);
        }

        if (y - 1 >= 0 && current >= heights[y - 1][x])
        {
            map[x, y] |= Flow(heights, x, y - 1, map);
        }

        if (y + 1 < heights.Length && current >= heights[y + 1][x])
        {
            map[x, y] |= Flow(heights, x, y + 1, map);
        }
